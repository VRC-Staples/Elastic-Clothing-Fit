...
pct = (clipping / len(fitted_indices) * 100) if fitted_indices else 0.0
print(f"  [PASS] clipping check completed (checked {len(fitted_indices)} vertices)")
if clipping > 0:
    print(f"  [WARN] {clipping} / {len(fitted_indices)} ({pct:.1f}%) fitted vertices closer than offset ({offset:.4f}m) to body surface")
'''
