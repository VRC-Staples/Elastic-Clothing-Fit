# tests/test_deployment_install.py
#
# Deployment install: install addon and leave it enabled in Blender.
# Runs steps 1-4 only (no disable or uninstall).
#
# Run via:
#   blender --background --python tests/test_deployment_install.py -- --zip <path>
#
    ...
    if "--" in argv:
        ...
        for i, a in enumerate(rest):
            if a == "--zip" and i + 1 < len(rest):
                return rest[i + 1]
    print("[FAIL] --zip argument not provided to install script")
    sys.exit(1)
