...
Now offset fine-tuning is applied to fitted vertices *before* the follow step. When the follow step reads the current positions of nearby fitted vertices to compute how far to drag each preserved vertex, those positions already include the offset contribution. The preserved vertices themselves are never directly pushed by the offset — the adjustment reaches them indirectly through the follow mechanism, exactly as the shrinkwrap displacement does.
...
- UV preservation
- Preserve group with KDTree-based displacement follow
